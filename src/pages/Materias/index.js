    ...
    <Theme>
      <Background1>
        <Container>
            ...
            renderItem={({item}) => <SquareButton data={item} onPress = {() => navigation.navigate('Home')}/>}
            />
        </Container>
        <ButtonContinuar />
      </Background1>
...
/*useEffect(() => {
    async function loadFiltro(){
      ...
      const storageFiltro = await AsyncStorage.getItem('filtros');
      
      if(storageFiltro){
        setFiltros(JSON.parse(storageFiltro));
      }
    }
    loadFiltro();
  },[]);*/
